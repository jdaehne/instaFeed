...
class InstaFeed {
     ...
     */
     public function __construct(modX &$modx, array $config = array())
     {
        ...
        $assetsUrl = $this->modx->getOption('signfy.assets_url',$config,$this->modx->getOption('assets_url').'components/instafeed/');
        ...
        $this->modx->addPackage('instafeed',$this->config['modelPath']);
    }
    ...
    public function getItems($limit, $offset, $sortby, $sortdir, $filterUser = NULL, $filterContent = NULL, $cache = array())
    {
        ...
        // query
        $c = $this->modx->newQuery('InstaFeedItem');
        $c->where($where);
        $c->sortby($sortby, $sortdir);
        $c->limit($limit, $offset);
        $items_array = $this->modx->getCollection('InstaFeedItem', $c);
    ...
    public function import()
    {
        ...
        if (is_array($this->usernames)) {
            foreach ($this->usernames as $username) {
                $items = array_merge($items, $this->getItemsFromInstagram($username));
            }
        }
        ...
        foreach ($items as $item) {
            ...
            $image = $this->copyImage($item['image'], $item['key'], $item['user_name']);
            if ($image === false) {
                continue;
            }

            // save to database
            $post = $this->modx->newObject('InstaFeedItem');
            $post->set('key', $item['key']);
            $post->set('user_name', $item['user_name']);
            $post->set('user_id', $item['user_id']);
            $post->set('type', $item['type']);
    ...
    private function getItemsFromInstagram($username)
    {
        ...
        $media_array = $insta_array['entry_data']['ProfilePage']['0']['graphql']['user']['edge_owner_to_timeline_media']['edges'];
    ...
    {
        return  $this->modx->cacheManager->get($key . '.instafeed');
    }

    // add items to cache
    public function addCache($items, $key, $time)
    {
        $this->modx->cacheManager->delete($key . '.instafeed');
        $this->modx->cacheManager->set($key . '.instafeed', $items, $time);
